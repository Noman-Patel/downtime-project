...
export function PlantSetupManager() {
  ...
  const remove = async (kind: "department" | "line", id: number, name: string) => {
  ...
  return <div className="mx-auto max-w-7xl">
    <div><p className="text-xs font-bold uppercase tracking-[.2em] text-cyan-700">Plant structure</p><h1 className="mt-2 text-3xl font-bold">Plant setup</h1><p className="mt-2 text-sm text-slate-500">Organize departments and the production lines where your machines operate.</p></div>
    ...
    {loading ? <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-16 text-center text-sm text-slate-500">Loading plant setup…</div> : <div className="mt-8 grid items-start gap-6 lg:grid-cols-2">
    ...
    {editor && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 p-4 backdrop-blur-sm"><form onSubmit={save} className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-center justify-between"><div><h2 className="text-xl font-bold">{editor.item ? "Edit" : "Add"} {editor.kind === "department" ? "department" : "production line"}</h2><p className="mt-1 text-sm text-slate-500">These values will be available across the application.</p></div><button type="button" onClick={() => setEditor(null)} className="text-2xl text-slate-400">×</button></div>
